...
test.group('Logger', () => {
	...
	test('log message with additional data', (assert) => {
		const logger = new Logger({ fake: true })
		assert.equal(logger.success('Hello %s', 'world'), 'underline(green(success)) Hello world')
	})

	test('log message without color', (assert) => {
		const logger = new Logger({ fake: true })
		assert.equal(logger.success({ message: 'Hello world', color: false }), 'success Hello world')
	})
