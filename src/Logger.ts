...
export class Logger {
	...
	private computeBiggestLabel() {
		this.biggestLabel = Math.max(
			...Object.keys(this.actions).map((name: keyof ActionsList) => {
	...
	private serializeError<T extends { stack?: any; name?: any; message?: any; code?: any }>(
		error: T
	): T {
	...
	protected $printMessage(message: DeferredMessageNode) {
		 ...
		 * icon and label
		 */
		const justifyWhitespace = this.getWhitespace(
			this.biggestLabel - stringWidth(`${icon}${label}`) + 2
		)
		this.$log(
			method,
	...
	public log(
		...
		messageNode: string | Error | MessageNode,
		...args: string[]
	) {
